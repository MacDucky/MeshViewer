...
void ShaderProgram::setUniform(const GLchar *name, const glm::vec3 v[], int size) {
    GLint loc = getUniformLocation(name);
    glUniform3fv(loc,size,glm::value_ptr(v[0]));
}

void ShaderProgram::setUniform(const GLchar *name, const glm::mat4 *m, int size) {
    GLint loc = getUniformLocation(name);
    glUniformMatrix4fv(loc,size,GL_FALSE,glm::value_ptr(m[0]));
}
