...
std::unique_ptr<MeshModel> Utils::LoadMeshModel()
{
    std::vector<Face> faces;
    std::vector<glm::vec3> vertices;
    std::vector<glm::vec3> normals;
    std::vector<glm::vec2> textureCoords;
    const std::string light_path = R"(C:\Users\Dany\CLionProjects\computergraphics2021-dp\Data\light_sphere.obj)";
    std::ifstream ifile(light_path.c_str());
    ...
    return std::make_unique<MeshModel>(faces, vertices, normals,textureCoords, Utils::GetFileName(light_path));
}
